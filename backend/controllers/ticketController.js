...
const { ensurePdfTicketFile } = require("../services/ticketService");
const {
  verifySignedQrData,
  getScanWindow,
...
const downloadTicketFile = async (req, res, next) => {
  try {
    ...
    const resolvedPath = await ensurePdfTicketFile(ticket);
    await fs.access(resolvedPath);
    res.setHeader("Content-Type", "application/pdf");
    res.download(resolvedPath, `${ticket.ticketCode}.pdf`);
  } catch (error) {
