...
const initiatePayment = async ({ bookingId, userId, idempotencyKey = null }) => {
  ...
  await bookingService.ensureBookablePaymentWindow(booking);
...
const confirmStripePayment = async ({ userId, transactionId }) => {
  ...
  if (
    String(intent.metadata?.bookingId || "") !== String(booking._id) ||
    String(intent.metadata?.userId || "") !== String(userId)
  ) {
    throw new ApiError(403, "Stripe payment intent does not belong to this booking");
  }
